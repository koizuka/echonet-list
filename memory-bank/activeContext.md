...
4. **Web UI 開発 (計画)**
    ...
    - **配信方法:** `echonet-list` アプリケーション自体にHTTPサーバー機能を追加し、Web UIのファイル（HTML/CSS/JS）を配信する計画です。これにより、WebSocketサーバーと同一オリジンからの配信となり、CORSの問題を回避できます。
        - Go標準の `net/http` パッケージを使用し、`http.FileServer` で静的ファイルを配信します。
        - HTTPサーバー用のポートは設定ファイル (`config.toml`) で指定可能にします（WebSocketポートとは別）。
    - **ディレクトリ構成:** プロジェクトルートに `webapp/` (または `frontend/`) ディレクトリを作成し、Web UIのソースコードを格納します。ビルド成果物はGoサーバーから配信可能な場所に配置します（例: `server/webroot`）。
    - **将来的な機能要件:**
