...
1.  **アーキテクチャ分割の完了 (最優先)**:
    *   ECHONET Liteに関する処理を完全にWebSocketサーバーに分離し、コンソールUIアプリケーションはクライアントとしてWebSocketサーバーに接続するようにアーキテクチャを完成させます。
    *   現在のコードの依存関係を整理し、サーバーとクライアントの責務を明確にします。
...
3.  **Web UI 開発 (計画)**:
    *   上記アーキテクチャ分割が完了次第、Web UIの開発に着手します。
...
現在は、アーキテクチャ分割の最終段階と、コンソールUI（WebSocketクライアント）の機能実装を進める必要があります。
