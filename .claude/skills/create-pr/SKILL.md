...
2. **ブランチ作成とコミット**:
   ...
   - **ステージング前のファイル確認**:
     - `git status --porcelain` の出力を確認し、意図しないファイル（`.env`, `*.log`, `*.pid`, 認証情報、巨大バイナリ、ローカル設定など）が含まれていないかチェック
     - `.gitignore` で抑止されているはずのファイルが Untracked に出ている場合はユーザーに警告して中断
     - 問題なければ `git add` で**個別パスを明示的に指定**してステージング（`git add .` は機密混入リスクがあるため避ける）
