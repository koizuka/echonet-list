...
デバイスがオフライン状態からオンラインに復旧したことを通知します。クライアントはこの通知を受けた後、デバイスの最新状態を取得することが推奨されます。
...
**クライアント実装時の注意事項:**
- この通知を受信した際は、デバイスの最新プロパティを取得するために `update_properties` を呼び出すことを推奨します
- オフライン時にデバイスデータを削除していた場合、次のプロパティ更新でデバイスが再度追加されます
